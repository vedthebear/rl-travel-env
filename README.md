...
Six rich-terminal dashboards, from trivial to complex. Each script auto-loads `.env` and gracefully skips itself if the API key isn't set.
...
Eight graduated tests, from simple to complex. `test_all.sh` runs them in order and stops on first failure.
...
notes.md         Informal scratch notes with fuller rationale behind every decision
```
...
Slot names like `"outbound_flight"`, `"hotel"`, `"activity_2"` are free-form strings. The env doesn't validate slot taxonomy. The reward function's `coherence` check does the structural work. This keeps the env permissive and the reward strict.
...
There is no `done` field in the obs. That's what `terminated` / `truncated` in the step return are for. This avoids the two-sources-of-truth trap.

### Persona: structured profile + LLM voice (the cleanest call I made)
...
- Hotels priced as `city_mult × (40 + 35·stars²) × neighborhood_premium × (1 + ε)`, with stars distributed `[5%, 15%, 35%, 30%, 15%]` for 1 to 5 stars and amenities sampled by tier.
- Activities have 5 hand-curated real anchors per city (Senso-ji Temple, Louvre, Colosseum, Tsukiji food tour, teamLab Planets, ...) plus 5 to 10 procedural fillers for inventory depth.

Constants are calibrated so a Tokyo 4-star night is ~$250 and an SFO to NRT economy round-trip is ~$700, which means a $40k trip looks absurd and a $1k trip looks tight. The agent's budget actually means something.

All randomness flows through a per-episode `numpy.random.Generator` derived from the world seed, never `np.random.`* directly. Identical seed produces a byte-identical world. Item IDs are stable hashes of the search arguments, so the same flight has the same ID across redundant searches.
...
**Dynamic weight renormalization.** Without a disruption, `recovery_quality` is `None` and the remaining three weights are renormalized to sum to 1. Otherwise no-disruption episodes would systematically score 25% lower for "lucking out", which would be perverse.

**The LLM judge is logged separately, never blended into the training reward.** Reasons I did not blend:
...
random       -0.013     0%   0.01    0.01   0.02    0/100        n/a (n=0)
...
## What I shipped from the optional enhancements

The take-home listed six prioritized enhancements. I landed three full and one partial:
...
- **Async + rate limiting.** Real API calls take 200 to 2000ms; rollouts would need a thread pool or asyncio. The env's `step` interface stays synchronous; the world module internally `await`s.
- **Result caching.** Search idempotence becomes load-bearing rather than nice-to-have. I don't want to pay for the same Amadeus query twice. The existing `_search_cache` in `world.py` already does this; it just becomes a hot path.
- **Pricing freshness.** Real prices change mid-conversation. I'd need either time-bounded cache TTLs or explicit "is this still bookable?" checks before `book`. The tentative/booked split makes this clean: re-validate just before committing.
...
- **Cost of training.** RL training against live APIs is expensive (per-call cost times millions of rollouts). The synthetic env stays useful as a pre-training environment; live APIs are for fine-tuning + eval.
...
- Anti-correlated noise in flight pricing, so cheap-and-fast-and-comfy is rare, so the agent has to confront tradeoffs.
...
The disruption-dodge finding is my favorite emergent result. I didn't design for it. Cheapest happens to escape disruptions by finishing fast. That's not a bug; it's an honest dynamic of the env, and I left it in. The reward function still penalizes cheapest on preferences, so the ordering holds; and if a disruption *does* fire and cheapest ignores it, the triple-penalty defense kicks in correctly. Sometimes the right thing to do is observe what the env produces and decide whether to fix it or accept it. I accepted it.